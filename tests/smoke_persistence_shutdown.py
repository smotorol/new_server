...
def main() -> int:
    ...
    persistence_needles = [
        ...
        "svr::dqs::is_version_conflict(expected_version, actual_version)",
        "++conflicts;",
    ...
    enter_pending_needles = [
        "auto serial_it = world_enter_serial_by_sid_.find(sid);",
        "serial_it == world_enter_serial_by_sid_.end() || serial_it->second != serial",
    ]
    for needle in enter_pending_needles:
        if needle not in session_text:
            print(f"[FAIL] enter-pending-serial-guard: missing '{needle}'")
            ok = False

    login_notify_needles = [
        "pending_world_enter_by_world_token_",
        "pending_world_enter_by_login_session_",
        "OnWorldEnterSuccessNotify consumed pending ref without active login session.",
        "OnWorldEnterSuccessNotify consumed pending ref on session-map-miss.",
    ]
    for needle in login_notify_needles:
        if needle not in login_runtime_text:
            print(f"[FAIL] login-notify-fallback: missing '{needle}'")
            ok = False
