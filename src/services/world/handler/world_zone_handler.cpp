    ...
    std::uint16_t load_score, std::uint32_t flags, std::string_view server_name, bool accepted)
{
    ...
    const auto h = proto::make_header((std::uint16_t)pt_wz::WorldZoneMsg::zone_server_register_ack, (std::uint16_t)sizeof(ack));
    ...
    bool create_if_missing, bool dungeon_instance)
{
    ...
    auto h = proto::make_header((std::uint16_t)pt_wz::WorldZoneMsg::world_zone_map_assign_request, (std::uint16_t)sizeof(pkt));
    ...
    std::uint32_t instance_id, std::uint16_t zone_id)
{
    ...
    auto h = proto::make_header((std::uint16_t)pt_wz::WorldZoneMsg::world_zone_player_enter, (std::uint16_t)sizeof(pkt));
    return Send(dwProID, dwIndex, dwSerial, h, reinterpret_cast<const char*>(&pkt));
}
    ...
    std::uint64_t char_id, std::uint32_t map_template_id, std::uint32_t instance_id, std::uint16_t zone_id)
{
    ...
    auto h = proto::make_header((std::uint16_t)pt_wz::WorldZoneMsg::world_zone_player_leave, (std::uint16_t)sizeof(pkt));
    return Send(dwProID, dwIndex, dwSerial, h, reinterpret_cast<const char*>(&pkt));
}
