...
struct DbPool
{
    std::vector<nanodbc::connection> conns;
    std::atomic<std::uint32_t> rr{ 0 };
    nanodbc::connection& next() { return conns[rr++ % conns.size()]; }
};
...
namespace svr {
    ...
    private:
        ...
        std::shared_ptr<CNetworkEX> world_handler_;
        std::shared_ptr<CNetworkEX> login_handler_;
        std::shared_ptr<CNetworkEX> control_handler_;
