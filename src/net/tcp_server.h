...
namespace net {
	...
	public:
		...
		struct SendqStats {
			...
			std::uint64_t drop_bytes_total = 0;
			std::uint64_t app_tx_bytes_total = 0;
			std::uint64_t app_rx_bytes_total = 0;
		};
		...
		SendqStats collect_sendq_stats() const
		{
			...
			for (auto& w : slots_) {
				...
				st.drop_bytes_total += s->send_drop_bytes();
				st.app_tx_bytes_total += s->app_tx_bytes();
				st.app_rx_bytes_total += s->app_rx_bytes();
			}
			return st;
		}
